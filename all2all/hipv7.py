...
# 核心改动：移除了 hip_unpack_to_intermediate 和 hip_final_reduction_smem
# 增加了一个新的融合核函数 hip_fused_unpack_and_combine
CPP_WRAPPER = """
...
// Kernels 1-5 from hipv6 are unchanged as they are already highly optimized.
...
// NEW FUSED KERNEL: Replaces unpack_to_intermediate and final_reduction
    ...
    int experts_per_token)
{
    ...
    // Each BLOCK processes one received item/token.
    int tid = blockIdx.x;
    if (tid >= total_recv) return;

    // --- Part 1: Find metadata and data pointers (done by thread 0, stored in shared mem) ---
    ...
    if (threadIdx.x == 0) {
        ...
        s_src_token = meta_read_ptr[2];
        int src_k = meta_read_ptr[3];
        s_weight = weights[s_src_token * experts_per_token + src_k];
    }
    __syncthreads();

    // --- Part 2: All threads in the block cooperate to perform the weighted accumulation ---
    int src_rank = 0;
    #pragma unroll
    for (int i = 1; i < WORLD_SIZE + 1; ++i) if (tid >= s_recv_offsets[i]) src_rank = i;
    int intra_rank_idx = tid - s_recv_offsets[src_rank];
...
// C++ wrapper function definitions
...
void hip_fused_unpack_and_combine(torch::Tensor rb, torch::Tensor w, torch::Tensor otf, torch::Tensor rdo, torch::Tensor rmo, torch::Tensor rc, int t, int h, int e) {
    // Launch one block per received token
    fused_unpack_and_combine_kernel<<<t, BLOCK_SIZE>>>(
        (const unsigned char*)rb.data_ptr(), w.data_ptr<float>(), otf.data_ptr<float>(),
        rdo.data_ptr<int>(), rmo.data_ptr<int>(), rc.data_ptr<int>(),
        t, h, e);
}
"""
...
hip_module = load_inline(
    name='hip_all2all_fused_combine_v1', # Use a new name to avoid caching issues
...
# The Python host code remains synchronous and sequential, similar to hipv6.
# The performance gain comes from the fused kernel, not from host-level asynchronicity.
class HIPAllToAllFused:
    META_DIM = 5
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        ...
        send_meta_bytes = self.META_DIM * 4 * send_counts
        ...
        recv_meta_bytes = self.META_DIM * 4 * recv_counts
    ...
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor, expert_meta: torch.Tensor, expert_y: torch.Tensor, expert_num_tokens: torch.Tensor, num_tokens: int):
        ...
        # Same backward counting and packing logic as hipv6
        send_counts = torch.zeros(self.world_size, dtype=torch.int32, device=device)
        ...
        # *** NEW COMBINE LOGIC ***
        if total_recv > 0:
            # 1. Create a temporary float32 buffer for atomic accumulation
            out_tokens_f32 = torch.zeros_like(out_tokens, dtype=torch.float32)

            # 2. Calculate byte offsets for received data
            recv_offsets = torch.cumsum(recv_split_bytes, 0, dtype=torch.int32) - recv_split_bytes
            recv_data_byte_offsets = recv_offsets
            recv_meta_byte_offsets = recv_offsets + recv_data_bytes
            
            # 3. Call the single fused kernel
            hip_module.hip_fused_unpack_and_combine(
                recv_buf, weights, out_tokens_f32,
                recv_data_byte_offsets, recv_meta_byte_offsets,
                recv_counts, total_recv, cfg.hidden_dim, cfg.experts_per_token)
            
            # 4. Cast the final result back to the target dtype
...
def custom_kernel(data: input_t) -> output_t:
    ...
    hip_all2all = HIPAllToAllFused(cfg, rank, world_size)
