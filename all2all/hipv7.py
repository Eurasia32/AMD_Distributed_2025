...
hip_module = load_inline(
    ...
    extra_cuda_cflags=[
      ...
      "-std=c++20",
      # AMD特定优化
      "-mllvm", "-amdgpu-function-calls=false",  # 内联所有函数调用
      "-mllvm", "-amdgpu-early-inline-all=true", # 激进内联
      "-mllvm", "--slp-vectorize-hor-store=true",      # 水平向量化存储
      "-mllvm", "--amdgpu-enable-vopd=true",           # 启用VOPD指令
      "-mllvm", "--amdgpu-dce-in-ra=true",             # 寄存器分配中的死代码消除
      "-mllvm", "--amdgpu-prelink=true",               # 预链接优化
      "-mllvm", "--amdgpu-mode-register=true"          # 模式寄存器优化
    ],
)
