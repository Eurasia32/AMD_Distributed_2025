...
__global__ void permute_kernel(const __half* original_tokens, const int* indices, const int* dispatch_map,
    ...
    // Vectorized copy using int4. This assumes hidden_dim is a multiple of 8.
    // Each int4 transfer moves 16 bytes = 8 half-precision floats.
    const int4* src = reinterpret_cast<const int4*>(&original_tokens[token_idx * hidden_dim]);
    int4* dst = reinterpret_cast<int4*>(&send_buf[final_pos * hidden_dim]);
...
__global__ void reorganize_expert_kernel(const __half* recv_buf, const int* recv_meta,
    ...
    if (pos < max_recv) {
        ...
        const int4* src = reinterpret_cast<const int4*>(&recv_buf[tid * hidden_dim]);
        int4* dst = reinterpret_cast<int4*>(&expert_x[local_eid * max_recv * hidden_dim + pos * hidden_dim]);
        for (int h = 0; h < hidden_dim / 8; h++) {
            dst[h] = src[h];
        }
...
__global__ void gather_kernel(const __half* expert_y, const int* expert_meta, const int* expert_num_tokens,
    ...
    // Vectorized copy using int4
    const int4* src = reinterpret_cast<const int4*>(&expert_y[local_eid * max_recv * hidden_dim + token_idx_in_expert * hidden_dim]);
    int4* dst = reinterpret_cast<int4*>(&send_buf[final_pos * hidden_dim]);
    for (int h = 0; h < hidden_dim / 8; ++h) {
        dst[h] = src[h];
    }
...
// This kernel now correctly takes a float* output buffer for accumulation.
...
void hip_gather_tokens(torch::Tensor expert_y, torch::Tensor expert_meta, torch::Tensor expert_num_tokens,
    ...
    gather_kernel<<<blocks, threads>>>((const __half*)expert_y.data_ptr<at::Half>(), expert_meta.data_ptr<int>(), expert_num_tokens.data_ptr<int>(), send_offsets.data_ptr<int>(), temp_send_counts.data_ptr<int>(), (__half*)send_buf.data_ptr<at::Half>(), send_meta.data_ptr<int>(), num_local_experts, max_recv, hidden_dim, meta_dim);
}

// This wrapper now correctly expects the out_tokens tensor to be float32.
void hip_combine_tokens(torch::Tensor recv_meta, torch::Tensor recv_buf, torch::Tensor weights,
                       torch::Tensor out_tokens, int total_recv, int meta_dim, int experts_per_token) {
    int blocks = (total_recv + BLOCK_SIZE - 1) / BLOCK_SIZE;
...
hip_module = load_inline(
    name='hip_all2all_optimized_fp16_v3',
...
class HIPAllToAll:
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        ...
        # Kernels now handle native FP16, no more conversions needed here.
    ...
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor,
        ...
        if total_recv > 0:
            ...
            # Create a temporary float32 buffer for accumulation.
            out_tokens_f32 = torch.zeros_like(out_tokens, dtype=torch.float32)

            # Call the kernel with the float32 buffer.
            ...
            # Copy the result back to the original half-precision tensor.
